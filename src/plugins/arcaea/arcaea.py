...
clear_statuses = ["Track Lost", "Track Complete", "Full Recall",
                  "Pure Memory", "Easy Complete", "Hard Complete"]
...
async def getUserInfo(nickname: str = None, usercode: int = None) -> str:
    ...
    if info["status"] == -3:
        ...
        # 获取歌曲Logo，使用`MessageSegment`转换为可发送的类型。
        song_icon = ms.image(await get_song(info["content"]["recent_score"][0]["song_id"]))
        # 获取注册时间，使用转换函数，见src/tools/utils.py中的`convert_time`函数
        register_time = convert_time(account_info["join_date"])
        # 获取搭档图片，使用`MessageSegment`转换为可发送的类型。
        partner = ms.image(await get_char(account_info["character"]))
        play_time = convert_time(info["content"]["recent_score"][0]
                                 ["time_played"])  # 获取玩家最近一次玩的时间，使用上述函数进行转换。
