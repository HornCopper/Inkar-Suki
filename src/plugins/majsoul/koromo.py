...
async def get_records(name: str = None, mode: str = "8.9.11.12.15.16"):
    ...
    if data == {}:
        ...
        for i in data:
            ...
            template = template.replace("1st", sorted_players[0]["nickname"]).replace("$sc1", str(sorted_players[0]["score"])).replace("$gr1", str(sorted_players[0]["gradingScore"]))
            template = template.replace("2nd", sorted_players[1]["nickname"]).replace("$sc2", str(sorted_players[1]["score"])).replace("$gr2", str(sorted_players[1]["gradingScore"]))
            template = template.replace("3rd", sorted_players[2]["nickname"]).replace("$sc3", str(sorted_players[2]["score"])).replace("$gr3", str(sorted_players[2]["gradingScore"]))
            template = template.replace("4th", sorted_players[3]["nickname"]).replace("$sc4", str(sorted_players[3]["score"])).replace("$gr4", str(sorted_players[3]["gradingScore"]))
