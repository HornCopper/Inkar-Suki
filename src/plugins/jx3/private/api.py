...
from src.plugins.jx3.dungeon import VIEWS
...
async def get_dilu_data():
    ...
    html = html.replace("$customfont", font).replace("$tablecontent", content).replace("$randomsaohua", saohua).replace("$appinfo", f"的卢统计 · {server} · {appinfo_time}")
