...
async def _(event: GroupMessageEvent, state: T_State, argument: Message = CommandArg()):
    ...
    if len(args) not in [1, 2]:
        await cost_calc_matcher.finish(PROMPT.ArgumentCountInvalid + "\n参考格式：成本 <服务器> <关键词>")
    if len(args) == 1:
        server = None
        name = args[0]
    elif len(args) == 2:
    ...
    if server is None:
        await cost_calc_matcher.finish(PROMPT.ServerNotExist)
    item_data = await get_item_data(name)
    if not item_data:
        await cost_calc_matcher.finish("未找到该物品，请检查后重试！")
    if isinstance(item_data, list):
        ...
        for num, name in enumerate(item_data, start=1):
            ...
            if num == 20:
                break
        await cost_calc_matcher.send(msg)
    else:
        image = await JX3CostCalc(item_data, server).render_image()
        await cost_calc_matcher.finish(image)

@cost_calc_matcher.got("num")
async def _(event: GroupMessageEvent, state: T_State, num: Message = Arg()):
    ...
    if not check_number(num_):
        await cost_calc_matcher.finish("唔……输入的不是数字，取消搜索。")
    if int(num_) > len(data):
        await cost_calc_matcher.finish("唔……不存在该数字对应的搜索结果，请重新搜索！")
