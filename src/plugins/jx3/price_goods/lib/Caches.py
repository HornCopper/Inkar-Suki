...
CACHE_Goods = json.loads(read(cache_file_goods))  # 每次重启后从磁盘加载缓存
...
CACHE_Goods_PriceSummary = json.loads(read(cache_file_summary))  # 每次重启后从磁盘加载缓存
...
CACHE_Goods_PriceDetail = json.loads(read(cache_file_detail))  # 每次重启后从磁盘加载缓存
