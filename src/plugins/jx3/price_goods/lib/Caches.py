...
CACHE_Goods_PriceSummary: Dict[str, GoodsPriceSummary] = json.loads(read(cache_file_summary))  # 每次重启后从磁盘加载缓存
CACHE_Goods_PriceSummary = dict([[x, dict2obj(GoodsPriceSummary(), CACHE_Goods_PriceSummary[x])] for x in CACHE_Goods_PriceSummary])  # 转换为类
...
CACHE_Goods_PriceDetail: Dict[str, GoodsPriceDetail] = json.loads(read(cache_file_detail))  # 每次重启后从磁盘加载缓存
CACHE_Goods_PriceDetail = dict([[x, dict2obj(GoodsPriceDetail(), CACHE_Goods_PriceDetail[x])] for x in CACHE_Goods_PriceDetail])  # 转换为类
