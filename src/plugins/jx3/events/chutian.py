...
async def getChutianImg():
    ...
    for i in final_data:
        ...
        section = i["stage"]
        desc = i["desc"]
        table.append(template_chutian.replace("$time", hour + ":" + minute).replace("$map", at_map).replace("$place", site).replace("$icon", icon).replace("$section", section).replace("$desc", desc))
    final_table = "\n".join(table)
