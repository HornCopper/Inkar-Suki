...
class Tieguyi(Kungfu):
    @classmethod
    def with_internel_id(cls, internel_id) -> "Self | str":
        if int(internel_id) not in [10389, 101025]:
...
class TieguyiCalculator(BaseCalculator):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

    @property
    def kungfu(self) -> Tieguyi:
        kungfu = Tieguyi.with_internel_id(
