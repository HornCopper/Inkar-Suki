...
class Xisuijing(Kungfu):
    @classmethod
    def with_internel_id(cls, internel_id) -> "Self | str":
        if int(internel_id) not in [10002, 100069]:
...
class XisuijingCalculator(BaseCalculator):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

    @property
    def kungfu(self) -> Xisuijing:
        kungfu = Xisuijing.with_internel_id(
    ...
    def cw(self) -> bool:
        for each_equip in self.raw_equips:
            if each_equip["Name"] in ["诸恶断尽", "降魔境"]:
                return True
        return False
    ...
    async def image(self, loop_arg: dict[str, str]):
        ...
        for t in loop_talents:
            x, y, icon = (await Qixue.create({"name": t}, "洗髓经")).location or (
                "",
                "",
                "",
            )
