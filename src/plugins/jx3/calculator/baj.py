...
class Beiaojue(Kungfu):
    @classmethod
    def with_internel_id(cls, internel_id) -> "Self | str":
        if int(internel_id) not in [10464, 100994]:
...
class BeiaojueCalculator(BaseCalculator):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

    @property
    def kungfu(self) -> Beiaojue:
        kungfu = Beiaojue.with_internel_id(
    ...
    async def image(self, loop_arg: dict[str, str]):
        ...
        for t in loop_talents:
            x, y, icon = (await Qixue.create({"name": t}, "北傲诀")).location or (
                "",
                "",
                "",
            )
