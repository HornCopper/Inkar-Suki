...
from ._template import msgbox_shanhaixinjue, template_calculator_shanhaixinjue


async def generate_calculator_img_shanhaixinjue(server: str, name: str):
    role_data = await search_player(role_name=name, server_name=server)
    if role_data.format_jx3api()["code"] != 200:
        return [PROMPT.PlayerNotExist]
    data = await get_calculated_data(server, name, "山海心诀")
    if not data:
