...
def data_process(kf, data):
    ...
    for i in data["data"]["Equips"]:
        ...
        if i["Icon"]["SubKind"] in "投掷囊":
            final[10] = i
        if kf not in ["问水诀","山居剑意"]:
            if i["Icon"]["SubKind"] != "投掷囊" and i["Icon"]["Kind"] == "武器":
                final[11] = i
        else:
            if i["Icon"]["Kind"]["SubKind"] == "剑":
                final[11] = i
            if i["Icon"]["Kind"]["SubKind"] == "重剑":
                final[12] = i
    return final
...
def enchant_mapping(quailty):
    ...
    for i in list(data):
        if data[i][min] <= int(quailty) <= data[i][max]:
            return i
    

async def get_attr_main(server, id):
    ...
    uid = uid[0]
    equip_data = data_process(data)
    maxjl_list = []
    jl_list = []
    equip_list = []
    equip_icon_list = []
    equip_quailty = []
    score = data["MatchDetail"]["score"]
    basic = [score, id, kftosh(kf), school_body]
    messyqx = []
    for i in data["Person"]["qixueList"]:
    ...
    ver = versions[0]["name"]
    data = await get_api(f"https://data.jx3box.com/talent/{ver}.json")
    for i in messyqx:
        qx[find_qx(data, kf, i)] = i
    for i in range(13):
        for x in data["Person"]["qixueList"]:
            if x["name"] == qx[i]:
    ...
    for i in equip_data:
        if i == "":
            ...
            for x in i["ModifyType"]:
                ...
                filter_string = ["全","阴性","阳性","阴阳","毒性","攻击","值","成效","内功","外功"]
    ...
    if flag == 2:
        ...
        for i in panel:
            ...
            if i["name"] == "加速":
                values[11] = str(i["value"])
    img = await get_attr(kf, maxjl_list, jl_list, equip_list, equip_icon_list, equip_quailty, basic, qx, qx_icon, henchant, lenchant, fs, wcs_icon, wcs, values)
    return img
    

def get_fs(level: int):
    return ASSETS + "/wuxingshi/" + level + ".png"
...
def kungfu_mapping(kf):
    if kf in ["紫霞功","莫问","毒经","无方","冰心"]:
        return "根骨"
    elif kf in ["花间游","易筋经","焚影圣诀","太玄经"]:
        return "元气"
    elif kf in ["太虚剑意","问水诀","山居剑意","凌海诀","隐龙诀","分山劲"]:
        return "身法"
    elif kf in ["傲血战意","惊羽诀","北傲决","孤锋诀","笑尘诀"]:
        return "力道"
...
async def get_attr(kungfu: str, maxjl_list: list, jl_list: list, equip_list: list, equip_icon_list: list, equip_quailty: list, basic: list, qx: list, qx_icon: list, henchant: list, lenchant: list, fs: list, wcs_icon: str, wcs: str, attrs: list):
    attr = kungfu_mapping(kungfu)
    syst_bold = ASSETS + "/syst-bold.ttf"
    syst_mid = ASSETS + "/syst-mid.ttf"
    msyh = ASSETS + "/msyh.ttf"
    calibri = ASSETS + "/calibri.ttf"
    if attr in ["根骨","元气","力道","身法"]:
        ...
        objects = ["外防","内防","最大气血值","破招","御劲","闪避","招架","拆招","体质","破防","无双","加速"]
    else:
        raise ValueError("Unknown type of kungfu!")
    background = Image.open(get_bg(kftosh(kungfu)))
    draw = ImageDraw.Draw(background)
    flickering = Image.open(ASSETS + "/flicker.png").resize((38, 38))
    precious = Image.open(ASSETS + "/xy.png")
    full_jinglian = Image.open(ASSETS + "/jl.png")
    un_full_jinglian = Image.open(ASSETS + "/unjl.png")
    heavy_enchant = Image.open(ASSETS + "/henchant.png").resize((20, 20))
    little_enchant = Image.open(ASSETS + "/lenchant.png").resize((20, 20))

    # 心法图标
    background.alpha_composite(Image.open(get_kf_icon(kungfu)), (61,62))

    # 武器图标
    if kungfu not in ["问水诀","山居剑意"]:
        if equip_icon_list[11] != "":
            background.alpha_composite(equip_icon_list[11], (708, 587))
        ...
        if equip_icon_list[12] != "":
            background.alpha_composite(equip_icon_list[12], (708, 636))
            if maxjl_list[12] in [3,4,8]:
                background.alpha_composite(precious, (688, 635))
                if maxjl_list[12] == 8:
                    background.alpha_composite(flickering, (707, 635))

    # 装备图标 懒得写了全用的抚今（莫问120级稀世神兵）
    init = 48
    limit = 0
    for i in equip_icon_list:
        if i != "":
            background.alpha_composite(Image.open(i), (708, init))
        init = init + 49
        limit = limit + 1
        if limit == 11:
            break

    # 装备精炼 理论未满应用另一图标（un_full_jinglian） 此处全满
    init = 47
    for i in range(13):
    ...
    # 个人基本信息
    draw.text((85, 127), basic[0], fill =(0, 0, 0), font=ImageFont.truetype(calibri, size=18), anchor="mt")
    ...
    for i in equips:
        draw.text((940, init), "帽子", file = (255, 255, 255), font=ImageFont.truetype(msyh, size=12), anchor="lt")
        init = init + 49
    ...
    for i in range(13):
        background.alpha_composite(Image.open(get_fs(fs[i])), positions[i])
    ...
    for i in range(6):
        if henchant[i] == "":
            ...
            background.alpha_composite(heavy_enchant, (1044, y))
            draw.text((1068, y + 4), henchant[i], file = (255, 255,255), font=ImageFont.truetype(msyh, size=12), anchor="lt")

    # 五彩石
    background.alpha_composite(Image.open(wcs_icon), (1044, 604))
