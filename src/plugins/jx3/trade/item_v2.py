...
async def getItemDetail(item_name: str):
    ...
    publish_time = convert_time(datetime.datetime.strptime(item_data["publishTime"], "%Y-%m-%dT%H:%M:%S.000+0000"), "%Y-%m-%d") # 发行时间
    publish_count_limit = item_data["publishNum"] # 发行数量
    publish_time_limit = item_data["publishLimitTime"] # 发行时长
    binding_time_limit = item_data["limitTime"] if item_data["limitTime"] != None else None # 绑定时长
    raw_price = item_data["priceNum"] # 发行原价
    ...
    for zone in [["电信一区", "双线一区", "双线二区", "双线四区"], ["电信五区", "电信八区"], ["无界区"]]:
        ...
        for each_data in data["data"]["records"]:
            for server in servers:
                if each_data["belongQf3"] == server and Zone_mapping(server, True) in zone:
                    end_time = convert_time(datetime.datetime.strptime(each_data["publishTime"], "%Y-%m-%dT%H:%M:%S.000+0000"), "%Y-%m-%d")
                    price = str(each_data["priceNum"]) + "元"
...
async def getSingleItemPrice(item_name: str):
    basic_item_info = await getItemDetail(item_name)
    if basic_item_info == False:
        return ["唔……未收录该物品！"]
    standard_name = basic_item_info[0]
