...
dev_global_command_record: dict[str, str] = filebase_database.Database(f"{bot_path.common_data_full}global_command_record").value
...
async def dev_mgr_command_map(bot: Bot, event: GroupMessageEvent, args=Depends(Jx3Arg.arg_factory)):
    ...
    async def complete(target_map: dict, action: str):
        ...
        await dev_cmd_mgr_command_map.finish(msg)

    if arg_glo:
        if checker(str(event.user_id), 10) == False:
            await dev_cmd_mgr_command_map.finish(error(10))
