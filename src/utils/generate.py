...
from src.const.path import CACHE, TEMPLATES, build_path
...
from src.utils.ui_theme import build_ui_color_css, supports_ui_color

import uuid
import asyncio


STYLE_2_CSS = read(build_path(TEMPLATES, ["style_2.css"]))
...
async def generate(
    ...
    *,
    ...
    # Only local HTML generated by the bot is themed; external web screenshots keep
    # the source site's own appearance. Apply the shared styles after page styles.
    if is_html:
        additional_css = f"{additional_css}\n{STYLE_2_CSS}"
        if supports_ui_color(source):
            ...
            if event is not None:
                ...
                if user_id is not None:
                    settings = db.where_one(
                        PersonalSettings(), "user_id = ?", str(user_id), default=None,
                    )
                    if settings is not None:
                        additional_css += build_ui_color_css(settings.setting.ui_color)
