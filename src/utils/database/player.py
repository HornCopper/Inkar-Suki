...
async def get_role_id(roleName: str, serverName: str) -> str | None:
...
@overload
async def get_uid_data(role_id: str = "", server: str = "", role_name: str = "", msg: Literal[True] = True) -> str: ...

@overload
async def get_uid_data(role_id: str = "", server: str = "", role_name: str = "", msg: Literal[False] = False) -> RoleData: ...

async def get_uid_data(role_id: str = "", server: str = "", role_name: str = "", msg: bool = True) -> str | RoleData:
...
async def search_player(
    ...
    *,
    ...
    if player_data.roleId == "" and not local_lookup:
        uid = await get_role_id(roleName=role_name, serverName=server_name)
        if uid is None:
            return RoleData()
        player_data = await get_uid_data(uid, server_name, role_name, False)
        return player_data
    else:
        return player_data