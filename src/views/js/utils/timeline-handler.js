...
function get_item_stamp(i, item_time_getter) {
...
function check_label(i, prei, item_time_getter) {
...
function put_label(item, from_now) {
 ...
 */
function tag_single_item(list, index, item_time_getter) {
 ...
 * @return {*}
 */
function tag_all_item(list, item_time_getter) {
  if (!item_time_getter) item_time_getter = i => new Date((i.request && i.request.stampLeave) || i.create)
