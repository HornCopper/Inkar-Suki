...
class Database:
    ...
    def where_one(self, model: LiteModel, condition: str = "", *args: Any, default: Any = None) -> Union[LiteModel, Any, None]:
        """查询第一个
        Args:
            model: 数据模型实例
    ...
    def where_all(self, model: LiteModel, condition: str = "", *args: Any, default: Any = None) -> Union[list[Union[LiteModel, Any]], None]:
        """查询所有
        Args:
            model: 数据模型实例
            condition: 查询条件，不给定则查询所有
...
class GroupSettings(LiteModel):
    ...
    server: str = "" # 群聊绑定的剑网3服务器
    group_id: str = "" # 群号
    subscribe: List[str] = [] # 订阅
    addtions: List[str] = [] # 功能附加
    welcome: str = "欢迎入群！" # 入群bbll
